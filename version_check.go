...
func GetLocalVersion(logger *logrus.Logger, binName, latestVer string) string {
...
func WhereIsBinInstalled(binName string) string {
	...
	for _, path := range []string{binName, path.Join("bin", binName), path.Join("out", binName)} {
...
func OutOfDateCheckLog(logger *logrus.Logger, binName, current, latest string) bool {
	if current == "" {
		logger.Error("Could not find "+binName+" binary", "install", GetInstallMsg(howToInstallBinary, latest))
		return true
	}
	...
	if isOutOfDate {
		logger.Error(
			...
			fmt.Sprintf("New %s version available ", binName),
			fmt.Sprintf("current:%s ", current),
			fmt.Sprintf("latest:%s ", latest),
			fmt.Sprintf("install `%s`", GetInstallMsg(howToInstallBinary, latest)),
		)
	}
	return isOutOfDate
}
...
func DoOutdatedNotificationRunCheck(logger *logrus.Logger) bool {
	now := time.Now()
...
func WriteLastTimeToFile(logger *logrus.Logger, lastCheckFile string, t time.Time) error {
...
func GetLatestVersionCheckFile(logger *logrus.Logger) (string, error) {
